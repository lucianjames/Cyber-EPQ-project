...
\item Section \ref{PP} will introduce protocols used to authenticate users, ranging from the very simplistic to much more advanced.
...
\subsection{A basic password authentication protocol (PAP)}
...
Encrypted key exchange is a process where two parties establish a shared private key. Once this private key has been created it can be used for symmetric encryption of further communications, meaning a password (or other such values such as in CHAP) can be sent from the user to the server with no risk of the password being exposed to any third party that is intercepting communications between the user and the server.
