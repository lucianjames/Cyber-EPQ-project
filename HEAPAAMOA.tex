...
\begin{itemize}
\item If the hosts password file is captured and the intruder learns the value of $v$, it should still not allow the intruder to impersonate the user without a potentially expensive dictionary search to find the value of $P_1$.
...
\item Public-key infrastructure is not required. Resolving the issues listed in section \ref{GEP}.
\item The SRP protocol is simple, and there is working code in OpenSSL that even integrates with TLS which makes it relatively easy to adopt.
\end{itemize}
...
\item Unlike SRP, OPAQUE has a reasonable security proof. According to \cite{green2018pake}, the security proof in the original SRP paper is not extensive enough, whereas the proof in the OPAQUE paper is extensive enough to be considered reasonably secure. The security proof issues with the original SRP were addressed in later revisions of the protocol and the paper
\end{itemize}
The main advantage is the fact OPAQUE does not reveal the salt to attackers. Revealing the salt allows hackers to perform pre-computation attacks which speed up the process of decrypting stolen hashed passwords. Performing this pre-computation allows the attackers to get usable passwords in less time from a breach of a servers hashed password files, giving administrators less time to react.\\\\
...
To compute this, the PRF requires a protocol between the client and server. In this protocol, the client first computes $H(P)$ and then `blinds' this password by selecting a random scalar value $r$, and blinding the result to obtain $C=H(P)^r$. The client then sends the blinded value $C$ over to the server. This is secure because (in a prime-order group), the blinding by $r$ hides all partial information about $P$.\\\\
