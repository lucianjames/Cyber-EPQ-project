...
Theoretically, the salt can be found. The client knows the value of $R'$ which is equal to $H(P)^s$, and the value of $H(P)$, so theoretically it is possible to find the value of $s$ by computing $log_{H(S)}R'$. But this is only a theoretical possibility, as this calculation falls into the discrete logarithm problem, which is well known to be impossibly difficult to practically compute \cite{dong2016math}. The difficulty to compute this relies on the value produced by $H(P)$ being a very large prime number (usually 1024 bits or more) with a large prime factor.
