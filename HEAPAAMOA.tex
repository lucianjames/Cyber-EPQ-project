...
\item The client computes $S_1=(B-g^{x_2})^{a+ux_2}$, and the server computes $S_2=(Av^u)^b$. If $S_1=S_2$, then $P_1=P_2$ therefore it is proven that the client knows the correct password.
\begin{proof}[Proof $S_1=S_2 \Longrightarrow P_1=P_2$]
\begin{align*}
S_2&=(Av^u)^b\\
&=(g^a((g^{H(s,P_1)})^u))^b\\
&=(g^a(g^{u(H(s,P_1))}))^b\\
