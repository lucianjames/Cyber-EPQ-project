...
\textbf{I think there might be some mistake in this relating to $R$, because:}
$R=C^s=(H(P)^r)^s$, $s$ is supposed to be hidden from the client, but could they not figure it out by doing $s=log_CR$? Since $R$ and $C$ are known to the client. Probably something to do with cyclic groups that the blog post \cite{green2018pake} doesnt mention for some reason.
