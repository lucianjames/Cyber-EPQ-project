...
In the simplest form of a password authentication protocol, the user/client sends to the host/server their plaintext username and password, then the server verifies the password, either by comparing it directly to a stored plaintext password or applying a one-way hash function to the received password and comparing it to a stored hash. Since the users plaintext password is immediately exposed to being intercepted, this method is unacceptable for use on untrusted networks.
...
\subsection{Basic diffie-hellman key exchange} \label{BDHKE}
\emph{Diffie-hellman key exchange is not in fact a password protocol}, but it is included in this section as it (or any other key exchange protocol) can be an incredibly important aspect of a password protocol (may be a component of protocols included in this section but i havent written about them yet).\\\\
Key exchange is where two parties exchange or agree on a shared secret which can then be used to secure further communications cryptographically.
Diffie-hellman key exchange is a method that two parties can use to securely establish a shared cryptographic key over an insecure network, which can be used to symmetrically encrypt data. The ability to establish a shared secret across an insecure network means that passwords and other such data which needs to be transmitted securely can indeed be transmitted.
Having securely encrypted communications between the server and the client means that in theory, a very simplistic protocol such as PAP can be used without great concern for security.
