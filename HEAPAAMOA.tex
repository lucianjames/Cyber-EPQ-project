...
\end{center}
% Should probably conclude something from this lmao
%top2bill diff = 1.055
%hk diff = 1.539
%chhc diff = 1.42
%wp diff = 1.751
%aslm diff = 8.81
The below table shows the result of $\sum{|x_i-y_i|}$ performed using the data about the password datasets, and the real-world password data. This gives a number quantifying how close each dataset is to real-world passwords in terms of character frequency.
...
\begin{center}
\makebox[\textwidth]{\includegraphics[width=\textwidth]{hashcatScript}}
\end{center}
...
\begin{center}
\makebox[\textwidth]{\includegraphics[width=10cm]{junkData}}
\end{center}
...
\cite{juels2013honeywords} describes the concept of ``honeywords''. this concept extends on honeypot accounts, aiming to address the issues identified with honeypot accounts.
...
Preventing active attacks requires much more additional cryptographic nonsense used to validate the identity of each party during communications, this additional cryptography is unfortunately not within the scope of this project.
